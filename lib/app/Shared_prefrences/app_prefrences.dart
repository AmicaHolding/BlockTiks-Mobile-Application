...
class AppPreferences {
  ...
  static const String prefAdsData = "ADS_DATA";
  ...
  static const String prefNotificationEventTitle =
      "PREF_NOTIFICATION_EVENT_TITLE";
  ...
  void setDeviceId({required String deviceId}) => _setPreference(
      prefName: prefDeviceId, prefValue: deviceId, prefType: prefTypeString);

  Future<String> getDeviceId() async => await _getPreference(
      prefName:
          prefDeviceId); // Check value for NULL. If NULL provide default value as FALSE.
  ...
  void setUserId({required String userId}) => _setPreference(
      prefName: prefUserData, prefValue: userId, prefType: prefTypeString);

  Future<String> getUserId() async =>
      await _getPreference(prefName: prefUserId);

  void setUserName({required String userName}) => _setPreference(
      prefName: prefUserName, prefValue: userName, prefType: prefTypeString);

  Future<String> getUserName() async =>
      await _getPreference(prefName: prefUserName);

  void setUserImage({required String userImage}) => _setPreference(
      prefName: prefUserImage, prefValue: userImage, prefType: prefTypeString);

  void setFirstLogin({required bool firstLogin}) => _setPreference(
      prefName: prefFirstLogin, prefValue: firstLogin, prefType: prefTypeBool);

  void setShowOnBoarding({required bool showOnBoarding}) => _setPreference(
      prefName: prefShowOnBoarding,
      prefValue: showOnBoarding,
      prefType: prefTypeBool);

  Future<bool> getShowOnBoarding() async =>
      await _getPreference(prefName: prefShowOnBoarding);

  Future<bool> getFirstLogin() async =>
      await _getPreference(prefName: prefFirstLogin);

  Future<String> getUserImage() async =>
      await _getPreference(prefName: prefUserImage);

  void setPayload({required String? data}) => _setPreference(
      prefName: prefNotificationData,
      prefValue: data,
  ...
  //       jsonDecode(_preferences.getString(prefNotificationData)!);
  //   debugPrint('map $payloadData');
  //   var payload = ExtraPayload.fromJson(payloadData);
  //   return payload;
  // }

  void setRole({required String role}) => _setPreference(
      prefName: prefRole, prefValue: role, prefType: prefTypeString);

  void setUserPhoneNumber({required String phoneNumber}) => _setPreference(
      prefName: prefUserPhoneNumber,
      prefValue: phoneNumber,
      prefType: prefTypeString);

  Future<String> getUserPhoneNumber() async =>
      await _getPreference(prefName: prefUserPhoneNumber);

  void setUserCity({required String city}) => _setPreference(
      prefName: prefUserCity, prefValue: city, prefType: prefTypeString);

  Future<String> getUserCity() async =>
      await _getPreference(prefName: prefUserCity);

  Future<String> getRole() async => await _getPreference(prefName: prefRole);

  void setBrandId({required String brandId}) => _setPreference(
      prefName: prefBrandId, prefValue: brandId, prefType: prefTypeString);

  Future<String> getBrandId() async =>
      await _getPreference(prefName: prefBrandId);

  void setBrandUserId({required String brandId}) => _setPreference(
      prefName: prefBrandUserId, prefValue: brandId, prefType: prefTypeString);

  Future<String> getBrandUserId() async =>
      await _getPreference(prefName: prefBrandUserId);

  void setBrandName({required String brandName}) => _setPreference(
      prefName: prefBrandName, prefValue: brandName, prefType: prefTypeString);

  Future<String> getBrandName() async =>
      await _getPreference(prefName: prefBrandName);

  void setUserDetails({required String data}) => _setPreference(
      prefName: prefUserDetails, prefValue: data, prefType: prefTypeString);

  void setFirebaseToken({required String deviceId}) => _setPreference(
      prefName: prefFirebaseToken,
      prefValue: deviceId,
      prefType: prefTypeString);

  Future<String> getFirebaseToken() async => await _getPreference(
      prefName:
          prefFirebaseToken); // Check value for NULL. If NULL provide default value as FALSE.

  void setAccessToken({required String token}) => _setPreference(
      prefName: prefAccessToken, prefValue: token, prefType: prefTypeString);

  Future<String?> getAccessToken() async =>
      await _getPreference(prefName: prefAccessToken);

  void setProfileData({required String data}) => _setPreference(
      prefName: prefProfile, prefValue: data, prefType: prefTypeString);

  Future<String?> getProfileData() async =>
      await _getPreference(prefName: prefProfile);

  void setUserEmail({required String email}) => _setPreference(
      prefName: 'prefEmail', prefValue: email, prefType: prefTypeString);

  void setUserEmailForForgotPassword({required String email}) => _setPreference(
      prefName: prefEmailForForgotPassword,
      prefValue: email,
      prefType: prefTypeString);

  void setShowUserQR({required String value}) => _setPreference(
      prefName: showUserQR, prefValue: value, prefType: prefTypeString);

  Future<String> getUserQR() async =>
      await _getPreference(prefName: showUserQR);

  void setUserLoggedIn({required String loginValue}) => _setPreference(
      prefName: loginAttempt, prefValue: loginValue, prefType: prefTypeString);

  Future<String> getUserLoggedIn() async =>
      await _getPreference(prefName: loginAttempt);

  void setFcmToken({required String token}) => _setPreference(
      prefName: prefFcmToken, prefValue: token, prefType: prefTypeString);

  Future<String> getFcmToken() async =>
      await _getPreference(prefName: prefFcmToken);

  Future<String> getUserEmail() async =>
      await _getPreference(prefName: 'prefEmail');

  Future<String> getUserEmailForgotPassword() async =>
      await _getPreference(prefName: prefEmailForForgotPassword);

  void setLoginPlatform({required String platform}) => _setPreference(
      prefName: prefLoginPlatform,
      prefValue: platform,
      prefType: prefTypeString);

  Future<String> getLoginPlatform() async =>
      await _getPreference(prefName: prefLoginPlatform);
  ...
  void setHasNotification({required bool hasNotification}) => _setPreference(
      prefName: prefHasNotification,
      prefValue: hasNotification,
      prefType: prefTypeBool);
  ...
  void setNotificationPayload({required String data}) => _setPreference(
      prefName: prefNotificationPayload,
      prefValue: data,
      prefType: prefTypeString);
  ...
  void setAppleUserModel({required String data}) => _setPreference(
      prefName: prefAppleModel, prefValue: data, prefType: prefTypeString);
